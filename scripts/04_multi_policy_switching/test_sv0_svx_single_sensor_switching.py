...
def get_reward_combined_strategy_position_1(params_primary, state_vector, params_alternative, alternative_SV, single_sensor_pattern):

    sensor_pattern_0 = single_sensor_pattern
    ...
    storm_env = BasicEnv(swmm_file, state_vector)
    Storm_state, EP_LEN = storm_env.reset()

    for t in range (EP_LEN):
        if sensor_pattern_0[t] == 0:
            # no communication failures, using SVO-based policy for control
            input_state = Storm_state
            Orifice_setting = get_action(params_primary, input_state)
        else:
            input_state = np.array([Storm_state[1], Storm_state[2], Storm_state[3]])
            Orifice_setting = get_action(params_alternative, input_state)
...
def get_reward_combined_strategy_position_2(params_primary, state_vector, params_alternative, alternative_SV, single_sensor_pattern):

    sensor_pattern_1 = single_sensor_pattern
...
def get_reward_combined_strategy_position_3(params_primary, state_vector, params_alternative, alternative_SV, single_sensor_pattern):

    sensor_pattern_2 = single_sensor_pattern
    ...
    for t in range (EP_LEN):
        if sensor_pattern_2[t] == 0:
            # no communication failures, using SVO-based policy for control
            input_state = Storm_state
            Orifice_setting = get_action(params_primary, input_state)
        else:
            input_state = np.array([Storm_state[0], Storm_state[1], Storm_state[3]])
            Orifice_setting = get_action(params_alternative, input_state)
...
def get_reward_combined_strategy_position_4(params_primary, state_vector, params_alternative, alternative_SV, single_sensor_pattern):

    sensor_pattern_3 = single_sensor_pattern
...
def load_communication_pattern_single_sensor(single_sensor, duration, failure_probability, parallel_num):
    base_path = str(REPO_ROOT / "data_required" / "failure_patterns" / f"probability {failure_probability}") + "/"

    single_sensor_pattern = read_xlsx(base_path, single_sensor, duration, parallel_num)

    return single_sensor_pattern
...
def Net_shapes_vs_params(state_vector, policy_index, len_state, len_action):
    ...
    return net_shapes, network_params
...
if __name__ == "__main__":
    ...
    for xxx in range(10):
        ...
        if SV == "SV0":
            policy_index_primary = 289
            if failure_sensor == "T2":
                Single_sensor_pattern = load_communication_pattern_single_sensor("T2", duration, failure_probability, parallel_num)

                alternative_SV = "SV1"
                policy_index_alternative = 231


            if failure_sensor == "T3":
                Single_sensor_pattern = load_communication_pattern_single_sensor("T3", duration, failure_probability, parallel_num)

                alternative_SV = "SV2"
                policy_index_alternative = 308


            if failure_sensor == "T4":
                Single_sensor_pattern = load_communication_pattern_single_sensor("T4", duration, failure_probability, parallel_num)

                alternative_SV = "SV3"
                policy_index_alternative = 230


            if failure_sensor == "T6":
                Single_sensor_pattern = load_communication_pattern_single_sensor("T6", duration, failure_probability, parallel_num)

                alternative_SV = "SV4"
                policy_index_alternative = 365


        print("SV", SV, "failure_sensor", failure_sensor, "parallel_num", parallel_num, "duration", duration)



        # net_shapes_primary, net_params_primary
        # net_shapes_alternative, net_params_alternative
        # --------------------------------------------------------------------------------------
        net_shapes_primary, params_primary = Net_shapes_vs_params("SV0", policy_index_primary, LEN_STATE_primary, LEN_ACTION)
        net_shapes_alternative, params_alternative = Net_shapes_vs_params(alternative_SV, policy_index_alternative, LEN_STATE_alternative, LEN_ACTION)


        if failure_sensor == "T2":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_combined_strategy_position_1(params_primary, SV, params_alternative, alternative_SV, Single_sensor_pattern)
        if failure_sensor == "T3":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_combined_strategy_position_2(params_primary, SV, params_alternative, alternative_SV, Single_sensor_pattern)
        if failure_sensor == "T4":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_combined_strategy_position_3(params_primary, SV, params_alternative, alternative_SV, Single_sensor_pattern)
        if failure_sensor == "T6":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_combined_strategy_position_4(params_primary, SV, params_alternative, alternative_SV, Single_sensor_pattern)
        ...
        output_path = REPO_ROOT / "data_required" / "simulation_outputs" / "sv0_svx_single_sensor_switching" / failure_sensor / f"{duration}_pattern_{parallel_num}.xlsx"
