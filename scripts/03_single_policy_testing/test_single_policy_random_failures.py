...
def get_reward_per_event_LEN_1(network_params, state_vector, General_Patterns):

    Obs_pattern = General_Patterns[0]
...
def get_reward_per_event_LEN_3(network_params, state_vector, General_Patterns):
    Obs_pattern_0 = General_Patterns[0]
    Obs_pattern_1 = General_Patterns[1]
    Obs_pattern_2 = General_Patterns[2]
...
def get_reward_per_event_LEN_4(network_params, state_vector, General_Patterns):

    Obs_pattern_0 = General_Patterns[0]
    Obs_pattern_1 = General_Patterns[1]
    Obs_pattern_2 = General_Patterns[2]
    Obs_pattern_3 = General_Patterns[3]
    ...
    for t in range (EP_LEN):
        if t == 0:
            ...
            if Obs_pattern_3[t] == 1:
                input_state3 = last_step_state[3]
            else:
                input_state3 = Storm_state[3]
            input_state = np.array([input_state0, input_state1, input_state2, input_state3])
            last_step_state = input_state
...
def load_communication_pattern(SV, duration, failure_probability, parallel_num):
    ...
    if SV == "SV1":
        pattern_1 = read_xlsx(base_path, "T3", duration, parallel_num)
        pattern_2 = read_xlsx(base_path, "T4", duration, parallel_num)
        pattern_3 = read_xlsx(base_path, "T6", duration, parallel_num)
    ...
    if SV == "T3 only":
        ...
        PATTERN = [pattern_1]
    if SV == "T4 only":
        pattern_1 = read_xlsx(base_path, "T4", duration, parallel_num)
        PATTERN = [pattern_1]
    if SV == "T6 only":
        pattern_1 = read_xlsx(base_path, "T6", duration, parallel_num)
        PATTERN = [pattern_1]
...
if __name__ == "__main__":
    ...
    for seed in range(10):
        ...
        parallel_num = seed
        print("SV", SV, "|", "parallel_num", parallel_num)

        General_Patterns = load_communication_pattern(SV, duration, failure_probability, parallel_num)
        ...
        if LEN_STATE == 1:
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event_LEN_1(network_params, SV, General_Patterns)
        if LEN_STATE == 3:
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event_LEN_3(network_params, SV, General_Patterns)
        if LEN_STATE == 4:
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event_LEN_4(network_params, SV, General_Patterns)
        ...
        output_path = REPO_ROOT / "data_required" / "simulation_outputs" / "single_policy_random_failures" / f"probability_{failure_probability}" / SV.replace(" only", "") / f"{duration}_pattern_{parallel_num}.xlsx"
