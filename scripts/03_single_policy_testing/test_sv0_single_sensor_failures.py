...
def get_reward_per_event_position_1(network_params, state_vector, single_sensor_pattern):
    ...
    for t in range (EP_LEN):
        if t == 0:
            ...
            if sensor_pattern_0[t] == 1:
                input_state0 = last_step_state[0]
            else:
                input_state0 = Storm_state[0]

            input_state = np.array([input_state0, Storm_state[1], Storm_state[2], Storm_state[3]])
            last_step_state = input_state

        Orifice_setting = get_action(network_params, input_state)
...
def get_reward_per_event_position_2(network_params, state_vector, single_sensor_pattern):
    ...
    for t in range (EP_LEN):
        if t == 0:
            ...
            if sensor_pattern_1[t] == 1:
                input_state1 = last_step_state[1]
            else:
                input_state1 = Storm_state[1]

            input_state = np.array([Storm_state[0], input_state1, Storm_state[2], Storm_state[3]])
            last_step_state = input_state
...
def get_reward_per_event_position_3(network_params, state_vector, single_sensor_pattern):
    ...
    for t in range (EP_LEN):
        if t == 0:
            ...
            if sensor_pattern_2[t] == 1:
                input_state2 = last_step_state[2]
            else:
                input_state2 = Storm_state[2]

            input_state = np.array([Storm_state[0], Storm_state[1], input_state2, Storm_state[3]])
            last_step_state = input_state
...
def get_reward_per_event_position_4(network_params, state_vector, single_sensor_pattern):
    ...
    for t in range (EP_LEN):
        if t == 0:
            ...
            if sensor_pattern_3[t] == 1:
                input_state3 = last_step_state[3]
            else:
                input_state3 = Storm_state[3]

            input_state = np.array([Storm_state[0], Storm_state[1], Storm_state[2], input_state3])
            last_step_state = input_state
...
if __name__ == "__main__":
    ...
    SV = "SV0"
    failure_sensor = "T2"  # T2, T3, T4, or T6
    ...
    for xxx in range(10):
        ...
        if failure_sensor == "T2":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event_position_1(network_params, SV, Single_sensor_pattern)
        if failure_sensor == "T3":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event_position_2(network_params, SV, Single_sensor_pattern)
        if failure_sensor == "T4":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event_position_3(network_params, SV, Single_sensor_pattern)
        if failure_sensor == "T6":
            Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event_position_4(network_params, SV, Single_sensor_pattern)
        ...
        output_path = REPO_ROOT / "data_required" / "simulation_outputs" / "sv0_single_sensor_failures" / failure_sensor / f"{duration}_pattern_{parallel_num}.xlsx"
