...
def get_reward_per_event(network_params, state_vector):
    ep_r = 0
...
if __name__ == "__main__":
    ...
    SVs = ['SV0', 'SV1', 'SV2', 'SV3', 'SV4', 'T2 only', 'T3 only', 'T4 only', 'T6 only']

    for mmm in range (len(SVs)):
        SV = SVs[mmm]
        ...
        Weighted_CSO_Volume_series, Total_CSO_Volume_series = get_reward_per_event(network_params, SV)
        ...
        output_path = REPO_ROOT / "data_required" / "simulation_outputs" / "ideal_conditions" / f"{SV.replace(' only', '')}.xlsx"
